...
ハンドラーが内部的にディスパッチしたアクション（`ActionScope.dispatch` または `task { }` 内から）をアサートします。テスト自身がディスパッチしたアクションはフィルターで除外されます。
...
> **注:** フィルターは到着順に型等値比較で外部アクションを照合します。テストがアクション `X` をディスパッチし、ハンドラーが*同じアクション値* `X` を即座に再ディスパッチした場合、ハンドラーのディスパッチが「外部」として誤認され無視されます。このまれなケースでは、ハンドラーが異なるアクション型をディスパッチするよう修正するか、`skipAction()` で曖昧な発行をスキップしてください。
