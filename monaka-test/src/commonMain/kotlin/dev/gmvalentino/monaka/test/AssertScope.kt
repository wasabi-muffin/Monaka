 ...
 * from the underlying [dev.gmvalentino.monaka.core.Store]:
 ...
 */
@MonakaTestDsl
public class AssertScope<State : StateMarker, Action : ActionMarker, Effect : EffectMarker> internal constructor(
    ...
    public inner class StateScope<T : State>(public val state: T)
    public inner class ActionScope<T : Action>(public val action: T)
    public inner class EffectScope<T : Effect>(public val effect: T)

    // ── State ─────────────────────────────────────────────────────────────────

    /** Await the next state and assert equality with [state]. */
    public suspend fun expectState(state: State) {
        states.awaitItem() shouldBe state
    }

    /** Await the next state and assert it matches [T] and the optional [predicate]. */
    public suspend inline fun <reified T : State> expectState(noinline predicate: StateScope<T>.() -> Boolean = { true }) {
    ...
    public suspend fun expectEffect(effect: Effect) {
        effects.awaitItem() shouldBe effect
    }

    /** Await the next effect and assert it matches [T] and the optional [predicate]. */
    public suspend inline fun <reified T : Effect> expectEffect(noinline predicate: EffectScope<T>.() -> Boolean = { true }) {
    ...
    public suspend fun skipEffect() {
        effects.awaitItem()
    }

    /** Assert that no effect has been emitted yet. */
    public suspend fun expectNoEffects() {
    ...
    public suspend fun expectAction(action: Action) {
    ...
    public suspend inline fun <reified T : Action> expectAction(noinline predicate: ActionScope<T>.() -> Boolean = { true }) {
    ...
    public suspend fun skipAction() {
        actions.awaitItem()
    }

    /** Assert that no handler-initiated action has been emitted yet. */
    public suspend fun expectNoAction() {
